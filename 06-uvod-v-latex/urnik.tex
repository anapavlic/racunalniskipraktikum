...
\usepackage{enumitem}  % Paket za nadzor seznamov
\usepackage{amsmath}   % Za matematične simbole (če jih potrebuješ)
...
\begin{itemize}
   ...
   predaval.
\end{itemize} % Konec neoštevilčenega seznama
...
% Začetek neoštevilčenega seznama (itemize)
\begin{itemize}
...
\end{itemize}
% Konec neoštevilčenega seznama (itemize)
...
zato si je smiselno podrobneje ogledati posamezne korake.
   ...
   $j$-tem koraku označimo s $R_j$.

   Za vse proste termine $T_{ij}$ izračunamo ceno vstavljanja
   $C_{T_{ij}}$, ki ga sestavlja $P_i$, profesor, ki ta predmet predava,
   ...
   vstavljanja, da bomo $P_i$  na koncu vstavili v termin z najboljšo
   ceno vstavljanja.

   Ko preverimo vse proste termine vseh predavalnic, vstavimo $P_i$
...
Rešitev se ponuja kar sama - za naslednjih \textit{maxTabu} korakov prepovemo vsako spremembo, ki jo
opravimo v koraku optimizacije. Izbira konstante \textit{maxTabu} je odvisna od samega problema, saj
ne obstaja neki splošen \textit{maxTabu}, ki bi nas v vsakem primeru pripeljal iz lokalnega ekstrema.
...
Naj bo \textit{sprememba\_1} ter \textit{sprememba\_2} sprememba urnika, ki nas je pripeljala v lokalni maksimum. To
spremembo (npr. zamenjava \textit{sprememba\_1} in \textit{sprememba\_2}) sedaj shranimo v
seznam tabujev. V naslednjem koraku izvedemo spremembo \textit{sprememba\_3}, ki se prav tako shrani v
seznam prepovedanih sprememb. Podobno naredimo za vse spremembe v nadaljnih korakih. S
tem poskrbimo, da se lahko v isti lokalni maksimum vrnemo šele čez \textit{maxTabu} korakov, saj
se sprememba \textit{sprememba\_1}, ki vodi do lokalnega maksimuma, šele tedaj izbriše iz seznama
prepovedanih sprememb.

Če smo dosegli lokalni maksimum in vrednost kriterijske funkcije v \textit{maxTabu} korakih uspemo
...
\end{verbatim}% začetek okolja verbatims
